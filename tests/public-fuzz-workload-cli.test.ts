...
return static function ( array $input, array $args ): array {
  ...
  assert.equal(descriptorJson.actionFamilies.some((family: { runtimeActionTypes: string[]; commands: string[] }) => family.runtimeActionTypes.includes("editor_actions") && family.commands.includes("wordpress.editor-validate-blocks")), true)
  ...
  assert.equal(readinessJson.capabilities.runtimeActionTypes.includes("editor_actions"), true)
  assert.equal(readinessJson.capabilities.runtimeActionTypes.includes("editor_validate_blocks"), true)
  assert.equal(readinessJson.capabilities.commands.includes("wordpress.editor-actions"), true)
  assert.equal(readinessJson.capabilities.commands.includes("wordpress.editor-validate-blocks"), true)
