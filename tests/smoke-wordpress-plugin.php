...
$assert( 'browser Playground runtime tool mapper accepts provider-safe filesystem_write alias', function_exists( 'wp_codebox_browser_runtime_tool_name' ) && 'filesystem_write' === wp_codebox_browser_runtime_tool_name( 'filesystem_write' ) );
...
$runner_missing_payload      = array( 'artifacts' => array_merge( $runner_task_payload['artifacts'], array( 'entrypoint' => 'generic-output/missing.html' ) ) );
$runner_missing_diagnostics = function_exists( 'wp_codebox_browser_artifact_capture_diagnostics' ) ? array( 'artifact_capture' => wp_codebox_browser_artifact_capture_diagnostics( $runner_missing_payload, $runner_missing_entrypoint ), 'response' => array( 'status' => 'completed' ) ) : array();
$runner_missing_error       = function_exists( 'wp_codebox_browser_required_artifact_error' ) ? wp_codebox_browser_required_artifact_error( $runner_missing_payload, $runner_missing_entrypoint, $runner_missing_diagnostics ) : null;
$assert( 'browser Playground generated runner fails completed runs missing required artifact bundle', is_wp_error( $runner_missing_error ) && 'wp_codebox_browser_artifact_bundle_missing' === $runner_missing_error->get_error_code() && 'caller/generic-browser-artifact-bundle/v1' === ( $runner_missing_error->get_error_data()['contract_schema'] ?? '' ) && 'generic-output/missing.html' === ( $runner_missing_error->get_error_data()['artifact_capture']['entrypoint'] ?? '' ) && empty( $runner_missing_error->get_error_data()['artifact_capture']['entrypoint_exists'] ) );
...
$assert( 'runner records selected provider and model in run metadata', ! is_wp_error( $result ) && 'openai' === ( $result['run_metadata']['provider'] ?? '' ) && 'gpt-5.5' === ( $result['run_metadata']['model'] ?? '' ) );
