...
Runtime substrate should be mounted as MU plugins so it loads before user-visible plugins. A typical bootstrap mounts Agents API, Data Machine, Data Machine Code, and the selected provider plugin/runtime bridge, then invokes the sandbox agent through the `wp-codebox.agent-sandbox-run` recipe step generated by `agent-task-run`.
